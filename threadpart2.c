...
//These are the 2 global variables for the synchronization. The pthread_mutex_t serves as the lock and unlock of the program that allows
//the threads to synchronize. The pthread_barrier_t allows each of the threads to catch up and terminate at the same time. 
pthread_mutex_t mutexLock;
pthread_barrier_t catchUp;
...
int main(int argc, char *argv[])
{
	...
	int i;
	//Initialize the pthread_mutex_init with the address of the global variable.
	...
	//Initialize the pthread_barrier_init with the catchup address and the inputted number from the user.
	pthread_barrier_init(&catchUp, NULL, threadNum);
